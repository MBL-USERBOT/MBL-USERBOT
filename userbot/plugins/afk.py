...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = (
            f"__HE IS AFK SINCE__ `{total_afk_time}` \nKAHA HAI WO: HAMKO KYA PATA   "
            + f"\n\n__RUKO DAKH TA HAI KYA REASON DIYA HAI..__\n**REASON**: {reason}"
            if reason
...
@borg.on(mbl_cmd(pattern=r"afk ?(.*)", outgoing=True))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    global USER_AFK  # pylint:disable=E0602
