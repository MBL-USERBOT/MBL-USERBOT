...
from userbot.utils import register, errors_handler, mbl_cmd
...
@borg.on(mbl_cmd(pattern="(ban|unban) ?(.*)", allow_sudo=True))
async def _(event):
...
@borg.on(mbl_cmd(pattern="pgs ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(mbl_cmd(pattern="(ban|unban) ?(.*)"))
async def _(event):
    # Space weirdness in regex required because argument is optional and other
    # commands start with ".unban"
    if event.fwd_from:
