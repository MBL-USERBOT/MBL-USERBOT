...
from userbot.utils import mbl_cmd, humanbytes, progress, time_formatter
from userbot.uniborgConfig import Config
thumb_image_path = Config.TMP_DOWNLOAD_DIRECTORY + "/thumb_image.jpg"
import io
from userbot.utils import mbl_cmd, progress
...
@borg.on(mbl_cmd(pattern="download(?: |$)(.*)", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
...
#@borg.on(mbl_cmd(pattern="uploadir (.*)", allow_sudo=True))
...
#@borg.on(mbl_cmd(pattern="upload (.*)", allow_sudo=True)
#@register(pattern=r".upload (.*)", outgoing=True)
@borg.on(mbl_cmd(pattern="upload (.*)", outgoing=True))                
async def _(event):
    if event.fwd_from:
        return
    mone = await event.reply("Processing ...")
...
#@borg.on(mbl_cmd(pattern="uploadas(stream|vn|all) (.*)", allow_sudo=True))
