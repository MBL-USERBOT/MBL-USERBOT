...
from userbot.utils import  errors_handler, mbl_cmd

@borg.on(mbl_cmd(pattern="leave$"))
async def leave(e):
...
@borg.on(mbl_cmd(pattern="chinfo(?: |$)(.*)", outgoing=True))
...
@borg.on(mbl_cmd(pattern="adminlist", outgoing=True))
@errors_handler
...
@borg.on(mbl_cmd(pattern=r"users ?(.*)", outgoing=True))
