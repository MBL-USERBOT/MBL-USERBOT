...
from userbot.utils import mbl_cmd, sudo_cmd
...
@borg.on(mbl_cmd(outgoing=True, pattern="getpaste(?: |$)(.*)"))
...
@borg.on(mbl_cmd(pattern="iffuci ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
...
@borg.on(mbl_cmd(pattern="paster ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    datetime.now()
